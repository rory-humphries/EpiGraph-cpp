#include "epigraph/epigraph/random_matrix.hpp"
#include "epigraph/epigraph/sixrd_odes.hpp"
#include "epigraph/toml11/toml.hpp"
...
int main(int argc, char *argv[]) {
  try {
    ...
    if (argc < 2) {
      ...
      std::cout << "Incorrect number of command line arguments passed. Only "
                   "the config file path is expected.\n";
      return -1;
    }
    ...
    for (int current_phase_idx = 0; current_phase_idx < phase_order.size();
      ...
      for (int tau = 0; tau < duration; tau++, t++) {
        ...
        std::cout << "R0 : " << R0 << "\n";
        std::cout << "Run time : " << time_span.count()
                  << "s , Total run time : " << run_time << "s\n";
        std::cout << "#################################\n\n";
