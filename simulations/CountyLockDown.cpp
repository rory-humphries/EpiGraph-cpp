...
int main(int argc, char *argv[]) {
	...
	if (argc < 2) {
		...
		std::cout
				<< "Incorrect number of command line arguments passed. Only the config file path is expected.\n";
		return -1;
	}
	print_banner2();
	...
	const std::string travel_weights_path = toml::find<std::string>(file_paths,
																	"travel_distribution");
	const std::string commuter_distribution_path = toml::find<std::string>(file_paths,
																		   "commuter_distribution");
	...
	const auto &compliance_list = toml::find<std::vector<double>>(config, "parameters",
																  "compliance_list");
	const auto &max_dist_list = toml::find<std::vector<double>>(config, "parameters",
																"max_dist_list");
	...
	MatrixXd new_travel_weights = (bool_mat).select(travel_weights.array(), travel_weights.array() *
																			(1 -
	...
	for (int t = 0; t < max_t; t++) {
		...
		VectorXd travel_pop = pop.unaryExpr([&](double x) {
		...
		max_dist_mat = general_outer_product(cur_max_dist, cur_max_dist, [](double a, double b) {
			return std::min(a, b);
		});
		bool_mat = distance_mat.array() < max_dist_mat.array();
		new_travel_weights = (bool_mat).select(travel_weights.array(), travel_weights.array() * (1 -
																								 cur_compliance.rowwise().replicate(
																										 travel_weights.cols()).array()));
		rnd_travel.set_row_distributions(new_travel_weights);
