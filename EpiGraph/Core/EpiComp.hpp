...
template <> struct traits<EpiComp> {
...
public:
  using state_type =
      Eigen::Matrix<typename StateMat::Scalar, StateMat::RowsAtCompileTime,
                    StateMat::ColsAtCompileTime>;
  using param_type =
      Eigen::Matrix<typename StateMat::Scalar, StateMat::RowsAtCompileTime,
                    StateMat::ColsAtCompileTime>;
  ...
  auto params() const -> const state_type & { return m_state; }
...
public:
  ...
  explicit EpiComp(Eigen::Index num_comps)
      : m_state(state_type::Zero(num_comps, 1)),
