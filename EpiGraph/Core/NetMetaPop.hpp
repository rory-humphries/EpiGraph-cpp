...
namespace EpiGraph {
	...
	public:
		...
		NetMetaPop(Eigen::Index dim, Eigen::Index compartments, Eigen::Index num_params) {
			...
			m_params = param_type::Zero((param_type::IsVectorAtCompileTime == 1 ? 1 : dim),
										num_params);
		...
		auto
		move_state(Eigen::Index comp1, Eigen::Index comp2, Eigen::Index i, double val) -> void {
			using Scalar = typename state_type::Scalar;
			Scalar change = m_state(i, comp1) -
		...
		auto set_params(Eigen::Index i, double val) -> typename std::enable_if<
			...
			if ((val < 0) || (val > 1))
				...
				m_params.col(
						i) = Eigen::Matrix<typename param_type::Scalar, param_type::RowsAtCompileTime, 1>::Ones(
						m_params.rows());
		}
		...
		auto set_params(Eigen::Index i,
						const Eigen::Matrix<typename ParamType::Scalar, Eigen::Dynamic, 1> &vec) -> void {
