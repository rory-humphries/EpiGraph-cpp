...
namespace EpiGraph {
    ...
    template<typename StateMat, typename ParamType, typename CoupType>
    class NetMetaPop : public NetEpiCompBase<NetMetaPop<StateMat, ParamType, CoupType>> {
    public:
        ...
        NetMetaPop(Eigen::Index dim, Eigen::Index compartments, Eigen::Index num_params) {
            m_state = state_type::Zero(dim, compartments);
            m_params = param_type::Zero((param_type::IsVectorAtCompileTime == 1 ? 1 : dim), num_params);
        ...
        auto move_state(Eigen::Index comp1, Eigen::Index comp2, Eigen::Index i, double val) -> void {
            ...
            Scalar change = m_state(i, comp1) - (comp1 > m_state(i, comp1) - val ? comp1 : m_state(i, comp1) - val);
        ...
        auto set_params(Eigen::Index i, double val) -> typename std::enable_if<
                T::IsVectorAtCompileTime == 1, void>::type {
            if ((val < 0) || (val > 1))
                throw std::domain_error("Encountered value greater than 1 or less than 0");
            else
                m_params[i] = val;
        ...
        auto set_params(Eigen::Index i, double val) -> typename std::enable_if<
            ...
            if ((val < 0) || (val > 1))
                ...
                m_params.col(i) = Eigen::Matrix<typename param_type::Scalar, param_type::RowsAtCompileTime, 1>::Ones(m_params.rows());
        }
        ...
        auto
        set_params(Eigen::Index i, const Eigen::Matrix<typename ParamType::Scalar, Eigen::Dynamic, 1> &vec) -> void {
