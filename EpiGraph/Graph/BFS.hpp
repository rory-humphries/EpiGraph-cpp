...
requires(Adj::IsRowMajor == 0) auto BFS(const Adj &adj, Eigen::Index v)
    -> Eigen::VectorXi {
...
template <IsSparseMatrix Adj>
auto BFS_parallel(const Adj &adj, Eigen::Index v) -> Eigen::VectorXi {
  ...
  std::vector<std::vector<int>> pvec(num_threads);
