...
auto SpectralRadius(Eigen::MatrixBase<Derived> mat, int ev = 6) {
    ...
    Spectra::GenEigsSolver<Scalar, Spectra::LARGEST_MAGN, Spectra::DenseGenMatProd<Scalar> > eigs(&op, 1, ev);
...
auto SpectralRadius(Eigen::SparseMatrixBase<Derived> mat, int ev = 6) {
    ...
    // Construct eigen solver object, requesting the largest
    // (in magnitude, or norm) three eigenvalues
    Spectra::GenEigsSolver<Scalar, Spectra::LARGEST_MAGN, Spectra::SparseGenMatProd<Scalar> > eigs(&op, 1, ev);
    // Initialize and compute
    eigs.init();
    int nconv = eigs.compute();
    // Retrieve results
