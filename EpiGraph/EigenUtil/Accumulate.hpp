...
auto accumulate_groups(const Eigen::MatrixBase<Derived> &vec1,
    ...
    -> std::map<T, typename Derived::Scalar> {
  /*
   * For each unique value in vec2, accumulate the elements in vec1 which match
   * indices with all the occurrences of a given element in vec2.
   */
  col_vector_assert(vec1);

  std::map<T, typename Derived::Scalar> group_sums;
  for (int i = 0; i < vec1.rows(); i++)
    group_sums[vec2[i]] += vec1[i];
