...
namespace EpiGraph {
	...
	auto
	general_outer_product(const Eigen::MatrixBase<Derived> &v, const Eigen::MatrixBase<Derived> &u,
						  F f) -> Eigen::Matrix<typename Derived::Scalar, Derived::RowsAtCompileTime, Derived::RowsAtCompileTime> {
		col_vector_assert(v);
		col_vector_assert(u);
		Eigen::Matrix<typename Derived::Scalar, Derived::RowsAtCompileTime, Derived::RowsAtCompileTime> mat(
				v.rows(), u.rows());
