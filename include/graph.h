...
class Network {
     ...
     */
public:
    using inner_adj_list_type = std::vector<Edge>;
    using adj_list_type = std::vector<inner_adj_list_type>;
    // iterators
    using AdjIter = adj_list_type::iterator;
    using OutEdgeIter = inner_adj_list_type::iterator;
    using InEdgeIter = inner_adj_list_type::iterator;
    ...
    auto add_edge(Vertex v_src, Vertex v_dst) -> std::pair<Edge, bool> {
        ...
        bool out_edge_exists;
        std::tie(out_e_it, out_edge_exists) = out_edge(v_src, v_dst);
        if (out_edge_exists) return {e, false};
    ...
    auto out_edges(Vertex v) -> std::pair<OutEdgeIter, OutEdgeIter> {
        /*
         * Return an iterator that traverses all out edges from vertex v.check memoryusage
         */
        return std::make_pair(m_out_adj_list[v].begin(), m_out_adj_list[v].end());
    }
...
class DynNetwork {
     ...
     */
public:
    using inner_adj_list_type = std::set<Edge>;
    using adj_list_type = std::vector<inner_adj_list_type>;
    // iterators
    using AdjIter = adj_list_type::iterator;
    using OutEdgeIter = inner_adj_list_type::iterator;
    ...
    public:
        ...
        using pointer = Edge *;
        using reference = Edge &;
        using constreference = const Edge &;
        using iterator_category = std::forward_iterator_tag;

        EdgeIter(OutEdgeIter e_curr,
                 OutEdgeIter e_end,
                 AdjIter v_curr,
                 AdjIter v_end) : m_e_curr(e_curr), m_e_end(e_end),
                                                                   m_edges(v_curr),
                                                                   m_edges_end(v_end) {
            if (m_edges->empty()) {
                while (m_edges != m_edges_end && m_edges->empty()) {
    ...
    // Default constructor
    DynNetwork() : m_num_vertices(0), m_num_edges(0) {};
    ...
    auto add_edge(Vertex v_src, Vertex v_dst) -> std::pair<Edge, bool> {
        ...
        auto it_pair = m_out_adj_list[v_src].insert(e);
        bool out_edge_exists = it_pair.second;

        if(out_edge_exists) return {e, true};


        it_pair = m_in_adj_list[v_dst].insert(e);
        bool in_edge_exists = it_pair.second;
        OutEdgeIter out_e_it;
        eprop[e];
        return {e, false};
    }
    ...
    auto out_edge(Vertex v_src, Vertex v_dst) -> std::pair<OutEdgeIter, bool> {
         ...
         */
        Edge e(v_src, v_dst);
        auto e_begin = m_out_adj_list[v_src].find(e);
        auto e_end = m_out_adj_list[v_src].end();

        return {e_begin, e_begin != e_end};
    }
    ...
    auto in_edge(Vertex v_src, Vertex v_dst) -> std::pair<InEdgeIter, bool> {
        ...
        */
        Edge e(v_src, v_dst);
        auto e_begin = m_in_adj_list[v_dst].find(e);
        auto e_end = m_in_adj_list[v_dst].end();
    ...
    auto edge(Vertex v_src, Vertex v_dst) -> std::pair<Edge, bool> {
        ...
        return std::make_pair(e, e_begin != e_end);
    }
