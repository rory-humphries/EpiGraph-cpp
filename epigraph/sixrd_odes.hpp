...
net_SIXRD_ode_inhom(const Eigen::MatrixBase<DerivedA> &xmat, const Eigen::EigenBase<DerivedB> &adj_,
    ...
    dxdt.col(4) = sixrd_params.col(alpha_idx).cwiseProduct(xmat.col(1) + xmat.col(2));// D
    dxdt.col(3) = sixrd_params.col(mu_idx).cwiseProduct(xmat.col(1) + xmat.col(2));// R
    dxdt.col(2) = sixrd_params.col(kappa_idx).cwiseProduct(xmat.col(1)) -
                  sixrd_params.col(mu_idx).cwiseProduct(xmat.col(2)) -
                  sixrd_params.col(alpha_idx).cwiseProduct(xmat.col(2)); // X

    dxdt.col(1) = sixrd_params.col(beta_idx).cwiseProduct(
    ...
    dxdt.col(0) = -sixrd_params.col(beta_idx).cwiseProduct(
            sixrd_params.col(c_idx).cwiseProduct(new_s.cwiseProduct(idiff_ndiff_inv))) -
                  (S * (sixrd_params.col(beta_idx).cwiseProduct(sixrd_params.col(c_idx)).asDiagonal()) *
                   idiff_ndiff_inv); // S
