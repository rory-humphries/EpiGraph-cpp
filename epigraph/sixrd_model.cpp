...
auto SIXRDNetModelUniParams::set_state(const SIXRDNetModelUniParams::state_type &x) -> void {
    if ((x.array() < 0).any())
...
auto SIXRDNetModelUniParams::set_compartment(SIXRDNetModelUniParams::StateId comp,
...
auto SIXRDNetModelUniParams::set_compartment(SIXRDNetModelUniParams::StateId comp, Eigen::Index i, double val) -> void {
    if (val < 0)
        throw std::domain_error("Encountered negative number");
    else
        m_state(i, comp) = val;
...
auto SIXRDNetModelUniParams::add_infected(Eigen::Index i, double N) -> void {
    double change = m_state(i, 0) - (0 > m_state(i, 0) - N ? 0 : m_state(i, 0) - N);
    m_state(i, 0) -= change;
    m_state(i, 1) += change;
}

auto SIXRDNetModelUniParams::set_params(const SIXRDNetModelUniParams::param_type &params) -> void {
...
auto SIXRDNetModelUniParams::set_params(SIXRDNetModelUniParams::ParamId i, double val) -> void {
    if ((val < 0) || (val > 1))
        throw std::domain_error("Encountered value greater than 1 or less than 0");
    else
        m_params[i] = val;
...
auto SIXRDNetModelUniParams::set_coupling(const SIXRDNetModelUniParams::coupling_type &coup) -> void {
...
auto derivative(const SIXRDNetModelUniParams &model) -> SIXRDNetModelUniParams::state_type {
    return net_SIXRD_ode(model.state(), model.coupling(), model.params());
}

auto print_totals(const SIXRDNetModelUniParams &model) -> void {
...
auto write_state(const SIXRDNetModelUniParams &model, const std::string &id, std::string dir) -> void {
...
auto write_state_totals(const SIXRDNetModelUniParams &model, const std::string &path_to_file,
