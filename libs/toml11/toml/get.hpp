                ...
                throw type_error(detail::format_underline("toml::value: "
                                                          "bad_cast to std::chrono::system_clock::time_point", {
                                                                  {std::addressof(detail::get_region(v)),
                                                                          concat_to_string("the actual type is ",
                                                                                           v.type())}
                                                          }), v.location());
            }
        }
...
::toml::find<T>(::toml::find(v, detail::key_cast(k1)),
        detail::key_cast(k2), std::forward<Keys>(keys)
...
{
return
::toml::find<T>(::toml::find(std::move(v), detail::key_cast(k1)),
        detail::key_cast(k2), std::forward<Keys>(keys)
...);
}
...
{
return
get <detail::remove_cvref_t<T>>(std::move(v)
);
}
catch(...)
{
return
detail::remove_cvref_t<T>(std::forward<T>(opt)
);
}
}
...
template<typename C,
        ...
        template<typename ...> class V>
basic_value <C, M, V> &
        find_or(basic_value < C, M, V > &v,
const toml::key &ky, basic_value<C, M, V>
& opt)
{
if(!v.
...
return
basic_value <C, M, V>(std::move(tab.at(ky))
);
}
...
return
get_or(std::move(tab.at(ky)), std::forward<T>(opt)
);
}
