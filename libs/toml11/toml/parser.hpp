    ...
    namespace detail {
        ...
        parse_octal_integer(location<Container> &loc) {
            ...
            if (const auto token = lex_oct_int::invoke(loc)) {
                ...
                str.erase(str.begin());
                str.erase(str.begin()); // remove `0o` prefix
        ...
        parse_hexadecimal_integer(location<Container> &loc) {
            ...
            if (const auto token = lex_hex_int::invoke(loc)) {
                ...
                str.erase(std::remove(str.begin(), str.end(), '_'), str.end());
                str.erase(str.begin());
                str.erase(str.begin()); // remove `0x` prefix
        ...
        result<std::string, std::string> parse_escape_sequence(location<Container> &loc) {
            ...
            if (first == loc.end() || *first != '\\') {
                return err(format_underline("toml::parse_escape_sequence: ", {{
                                                                                      std::addressof(
                                                                                              loc), "the next token is not a backslash \"\\\""}}));
            }
            loc.advance();
            switch (*loc.iter()) {
                case '\\': {
        ...
        result<Value, std::string> parse_value(location<Container> &loc) {
            ...
            switch (type.unwrap()) {
                case value_t::boolean        : {
                    return parse_boolean(loc);
                }
                case value_t::integer        : {
                    return parse_integer(loc);
                }
                case value_t::floating       : {
                    return parse_floating(loc);
                }
                case value_t::string         : {
                    return parse_string(loc);
                }
                case value_t::offset_datetime: {
                    return parse_offset_datetime(loc);
                }
                case value_t::local_datetime : {
                    return parse_local_datetime(loc);
                }
                case value_t::local_date     : {
                    return parse_local_date(loc);
                }
                case value_t::local_time     : {
                    return parse_local_time(loc);
                }
                case value_t::array          : {
                    return parse_array<value_type>(loc);
                }
                case value_t::table          : {
                    return parse_inline_table<value_type>(loc);
                }
                default: {
        ...
        result<Value, std::string> parse_toml_file(location<Container> &loc) {
            ...
            while (loc.iter() != loc.end()) {
                ...
                if (const auto tabkey = parse_table_key(loc)) {
                    ...
                    const auto inserted = insert_nested_key(data,
                                                            value_type(tab.unwrap(), reg), keys.begin(), keys.end(),
                                                            reg);
                    if (!inserted) { return err(inserted.unwrap_err()); }
