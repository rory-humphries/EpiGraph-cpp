...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_get_or_exact, value_type, test_value_types) {
    ...
    TOML11_TEST_GET_OR_EXACT(offset_datetime,
                             (toml::local_date(2019, toml::month_t::Apr, 1), toml::local_time(12, 30,
                                                                                              45), toml::time_offset(9,
                                                                                                                     0)),
                             (toml::local_date(1999, toml::month_t::Jan, 2), toml::local_time(6, 0,
                                                                                              30), toml::time_offset(-3,
                                                                                                                     0))
    )
    {
    ...
    {
        const typename value_type::table_type init{{"key1", 42},
                                                   {"key2", "foo"}};
        const typename value_type::table_type opt{{"key1", 54},
                                                  {"key2", "bar"}};
        const value_type v(init);
        BOOST_TEST(init != opt);
        BOOST_TEST(init == toml::get_or(v, opt));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_get_or_move, value_type, test_value_types) {
    ...
    }
    {
        const typename value_type::table_type init{{"key1", 42},
                                                   {"key2", "foo"}};
        typename value_type::table_type opt{{"key1", 54},
                                            {"key2", "bar"}};
        value_type v(init);
        BOOST_TEST(init != opt);
        const auto opt_ = toml::get_or(std::move(v), std::move(opt));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_get_or_modify, value_type, test_value_types) {
    ...
    {
        typename value_type::table_type init{{"key1", 42},
                                             {"key2", "foo"}};
        typename value_type::table_type opt1{{"key1", 54},
                                             {"key2", "bar"}};
        typename value_type::table_type opt2{{"key1", 54},
                                             {"key2", "bar"}};
        value_type v(init);
        BOOST_TEST(init != opt1);
        toml::get_or(v, opt2) = opt1;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_get_or_fallback, value_type, test_value_types) {
    ...
    const table_type table{{"key1", 42},
                           {"key2", "foo"}};
