...
#include "libs/spectra/include/Spectra/SymGEigsSolver.h"
...
///	*** EXAMPLE
/// \code{.cpp}
/// #epigraph <Spectra/contrib/SymSparseEigsSolverLOBPCG.h>
///
///	// random A
///	Matrix a;
...
///
    ...
    private:
        ...
        void applyConstraintsInPlace(SparseMatrix &X, SparseMatrix &Y,
            ...
            SparseMatrix YBY_XYX = (Matrix(YBY).bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(
                    Matrix(BYX))).sparseView();
            X = X - Y * YBY_XYX;
        }
        ...
        int checkConvergence_getBlocksize(SparseMatrix &m_residuals, Scalar tolerance_L2,
        ...
        void compute(int maxit = 10, Scalar tol_div_n = 1e-7) {
            ...
            for (int iter_num = 0; iter_num < max_iter; iter_num++) {
                ...
                if (iter_num > 0) {
                    ...
                    gramA = stack_9_matricies(m_evalues.asDiagonal(), XAR, XAD, XAR.transpose(), RAR, RAD,
                                              XAD.transpose(), RAD.transpose(), DAD.transpose());
                    gramB = stack_9_matricies(Matrix::Identity(m_nev, m_nev), XBR, XBD, XBR.transpose(),
                                              Matrix::Identity(BlockSize, BlockSize), RBD, XBD.transpose(),
                                              RBD.transpose(), Matrix::Identity(BlockSize, BlockSize));
                } else {
                    gramA = stack_4_matricies(m_evalues.asDiagonal(), XAR, XAR.transpose(), RAR);
                    gramB = stack_4_matricies(Matrix::Identity(m_nev, m_nev), XBR, XBR.transpose(),
                                              Matrix::Identity(BlockSize, BlockSize));
                }
