...
// MatrixType is either Eigen::Matrix<Scalar, ...> or Eigen::SparseMatrix<Scalar, ...>
    ...
    public:
        ...
        Matrix matrix_U(int nu) {
            ...
            return m_mat * (m_evecs.leftCols(nu).array().rowwise() /
                            m_eigs->eigenvalues().head(nu).transpose().array().sqrt()).matrix();
        }
        ...
        Matrix matrix_V(int nv) {
            ...
            return m_mat.transpose() * (m_evecs.leftCols(nv).array().rowwise() /
                                        m_eigs->eigenvalues().head(nv).transpose().array().sqrt()).matrix();
        }
    };
